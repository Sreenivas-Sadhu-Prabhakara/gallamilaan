...
func TestPostgresStore_RoundTrip(t *testing.T) {
	...
	in, _ := json.Marshal(Input{OpeningCash: 2000, CashSales: 5000, CountedClosing: 7000})
	saved, err := store.Save(Record{Input: in, Headline: 0, Label: "exact"})
	if err != nil {
		t.Fatalf("save: %v", err)
	}
	if saved.ID == 0 {
	...
	if len(items) == 0 || items[0].ID != saved.ID || items[0].Label != "exact" {
